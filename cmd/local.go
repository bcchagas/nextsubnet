...
var localCmd = &cobra.Command{
	Use:   "local -n network -m mask [--ignore-list list | --ignore-file file]",
  ...
  nextsubnet local --network 10.0.0.1/22 --subnet-mask 25 --ignore-list 10.0.0.0/24,10.0.1.128/25

  # You can also pass in a file containing the subnets in use
  nextsubnet local --network 10.0.0.1/22  --subnet-mask 24 --ignore-file subnets.txt`,
	Args: cobra.NoArgs,
...
func init() {
	...
	localCmd.Flags().IPNetVarP(&flags.network, "network", "n", net.IPNet{},
		"(Required) Address of the network the subnet will be based of in CIDR notation e.g. 10.0.0.0/22")
	localCmd.Flags().IntVarP(&flags.subnetMask, "subnet-mask", "m", 0,
		"(Required) Mask size of the subnet to be found e.g. 24")
	localCmd.Flags().StringVar(&flags.ignoreList, "ignore-list", "",
		"List of subnets in CIDR notation separated by comma e.g. '10.0.0.0/24,10.0.1.0/24'")
	localCmd.Flags().StringVar(&flags.ignoreFile, "ignore-file", "",
		"File containing the list of subnets in CIDR notation separated by line")

	localCmd.MarkFlagRequired("subnet-mask")
	localCmd.MarkFlagRequired("network")
